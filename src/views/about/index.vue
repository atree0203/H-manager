<template>
	<div>
		...
		<div class="container">
			<div id="histogramPict2" style="width: 800px;height:400px;"></div>
		</div>
	</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			histogramPicture() {
				...
				let myChart = this.$echarts.init(document.getElementById('histogramPict'))
				// 绘制图表
				myChart.setOption({
					tooltip: {
						trigger: 'axis'
			...
			technologyPict() {
				...
				let myChart = this.$echarts.init(document.getElementById('technologyPict'))
				// 绘制图表
				myChart.setOption({
